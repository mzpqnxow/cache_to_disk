...
DISK_CACHE_DIR = expanduser(
    expandvars(getenv("DISK_CACHE_DIR", join_path(dirname(realpath(__file__)), "disk_cache")))
)
DISK_CACHE_FILE = expanduser(
    expandvars(
        join_path(DISK_CACHE_DIR, getenv("DISK_CACHE_FILENAME", "cache_to_disk_caches.json"))
    )
)
...
def cache_exists(cache_metadata: Dict, function_name: str, *args, **kwargs) -> Tuple[bool, Any]:
    if function_name not in cache_metadata:
        return False, None
